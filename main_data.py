...
from data_injest.loader2 import PDFLoaderDocling
...
def run_ingestion(config: dict) -> list:
    loader_type = config["ingestion"].get("loader", "pymupdf4llm")
    loader = PDFLoaderDocling(config) if loader_type == "docling" else PDFLoader(config)
...
def main():
    ...
    logger.info(f"Embedding done | {len(embedded_chunks)} chunks | dimensions={embedder.dimensions}")
