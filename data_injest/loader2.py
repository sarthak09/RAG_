...
def setup_logger(log_dir: str) -> logging.Logger:
    ...
    log_file = Path(log_dir) / f"ingestion_docling_{datetime.now().strftime('%Y%m%d_%H%M%S')}.log"
    logger = logging.getLogger("rag.loader2")
...
class PDFLoaderDocling:
    ...
    def load_all(self) -> list[ParsedDocument]:
        ...
        self.logger.info(f"Starting ingestion (Docling) | {len(pdf_files)} PDFs | {self.pdf_dir}")
    ...
    def _log_summary(self):
        s = self.stats
        self.logger.info("=" * 55)
        self.logger.info("INGESTION COMPLETE (Docling)")
    ...
    def _save_stats(self):
        stats_file = self.stats_dir / f"loader2_stats_{datetime.now().strftime('%Y%m%d_%H%M%S')}.json"
