...
def setup_logger(log_dir: str) -> logging.Logger:
    ...
    log_file = Path(log_dir) / f"Data_embedder_{datetime.now().strftime('%Y%m%d_%H%M%S')}.log"
...
class Embedder:
    def __init__(self, config: dict):
        ...
        self.model = OllamaEmbeddings(model=self.model_name, base_url=self.base_url)
        self.dimensions = self._detect_dimensions()
        self.logger.info(f"Embedder model={self.model_name} | dimensions={self.dimensions}")

    def _detect_dimensions(self) -> int:
        try:
            vector = self.model.embed_query("test")
            self.logger.info(f"Auto detected dimensions={len(vector)} for model={self.model_name}")
            return len(vector)
        except Exception as e:
            self.logger.error(f"Could not load model '{self.model_name}': {e}")
            raise
    ...
    def embed_one_by_one(self, chunks: list[Chunk]) -> list[EmbeddedChunk]:
        self.logger.info(f"Starting one-by-one embedding | total={len(chunks)}")
        ...
        for i, chunk in enumerate(chunks):
            try:
                ...
                if (i + 1) % 100 == 0:
                    self.logger.info(f"Progress: {i + 1}/{len(chunks)}")
            except Exception as e:
                failed += 1
                self.logger.error(f"Failed chunk {chunk.chunk_id}: {e}")
        elapsed = round(time.time() - start, 2)
        self.logger.info(f"One-by-one done | embedded={len(embedded)} | failed={failed} | time={elapsed}s")
    ...
    def embed_batch(self, chunks: list[Chunk]) -> list[EmbeddedChunk]:
        self.logger.info(f"Starting batch embedding | total={len(chunks)} | batch_size={self.batch_size}")
        embedded, failed, start = [], 0, time.time()
        for batch_start in range(0, len(chunks), self.batch_size):
            ...
            try:
                ...
                for chunk, vector in zip(batch, vectors):
                    embedded.append(self._to_embedded(chunk, vector))
                self.logger.info(f"Batch {batch_start // self.batch_size + 1} done | size={len(batch)}")
            except Exception as e:
                failed += len(batch)
                self.logger.error(f"Batch at {batch_start} failed: {e}")
        elapsed = round(time.time() - start, 2)
        self.logger.info(f"Batch done | embedded={len(embedded)} | failed={failed} | time={elapsed}s")
    ...
    def _save_stats(self, method: str, total: int, succeeded: int, failed: int, elapsed: float):
        ...
        filename = f"Data_embedder_stats_{datetime.now().strftime('%Y%m%d_%H%M%S')}.json"
