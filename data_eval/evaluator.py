...
class RetrieverEvaluator:
    ...
    def __init__(self, config: dict):
        ...
        logger.info("RetrieverEvaluator ready | store=%s | llm=%s",
    ...
    def evaluate(self, eval_dataset: list[dict]) -> dict:
        ...
        for i, item in enumerate(eval_dataset):
            ...
            if (i + 1) % 25 == 0:
                logger.info("Retrieval progress: %d / %d", i + 1, len(eval_dataset))
    ...
    # ─────────────────────────────────────────
    # Step 2 — Detailed results with LLM answer
    # ─────────────────────────────────────────
    ...
    def save_detailed_results(self, eval_dataset: list[dict]) -> list[dict]:
        ...
        For every query:
          ...
          - calls the LLM to generate an answer from those chunks
          - stores query + ground truth answer + LLM answer + context + metrics
        Saves to detailed_results_*.json and detailed_results_*.csv
        Original eval_dataset files are never touched.
        """
        if not self._last_results:
            logger.error("No results found. Run evaluate() first.")
            return []
        ...
        for i, r in enumerate(self._last_results):
            ...
            # Re-retrieve to get full chunk text
            ...
            # Call LLM using the retrieved context
            llm_answer = self._generate_answer(r["query"], retrieved_context)

            logger.info("Processed %d / %d | query: %s...", i + 1, total, r["query"][:60])
        ...
        logger.info("Detailed results (JSON) → %s", json_path)

        # CSV — flattened, one row per query
        ...
        logger.info("Detailed results (CSV)  → %s", csv_path)
        return detailed
    ...
    def _generate_answer(self, query: str, chunks: list[dict]) -> str:
        """Calls the LLM with the retrieved chunks and returns a plain text answer."""
        try:
            messages = [
            ...
            logger.error("LLM generation failed for query '%s': %s", query[:60], e)
...
if __name__ == "__main__":
    ...
    logger.info("Loaded %d evaluable queries", len(eval_dataset))
    ...
    # Step 2 — detailed results with LLM answers
